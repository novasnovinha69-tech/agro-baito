...
export function CheckoutCliente({ unidades }: Props) {
  ...
  //  - Retirada: número da loja selecionada (a loja selecionada)
  //  - Entrega: número da unidade responsável pela zona de entrega
  //  - Fallback: número principal
  ...
  async function finalizarComPix() {
    ...
    try {
      ...
      if (data.pix?.qrCode) {
        ...
        toast.success(
          `Pedido ${data.pedido.codigo} criado! Pague o Pix para confirmar.`,
        );
      } else {
      ...
      <div className="container-agro py-12">
        <div className="mx-auto max-w-md rounded-2xl border-2 border-agro-blue/30 bg-agro-blue/5 p-8 text-center">
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Pedido <strong>{pixGerado.pedidoCodigo}</strong> ·{" "}
          ...
          <div className="mt-5 rounded-lg bg-muted p-3 text-left text-xs text-muted-foreground">
            <p className="flex items-center gap-2">
              <Clock className="h-4 w-4 shrink-0" />O QR Code expira em 30
              minutos. Após o pagamento, a confirmação é automática.
            </p>
          </div>
        ...
        <div className="lg:sticky lg:top-6 lg:self-start">
          <Card>
            <CardContent className="p-5">
              ...
              {metodoPagamento === "nao_loja" && (
                <>
                  <Button className="mt-4 w-full" size="lg" onClick={finalizar}>
